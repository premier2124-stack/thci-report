...
# 리포트 원본 폴더명 자동 감지 (THCI데일리리포트 = 실제, TKS데일리리포트 = 구명칭 호환)
$sig = Join-Path $od "문서\시그널리포트"
$base = $null
foreach ($n in @("THCI데일리리포트", "TKS데일리리포트")) {
    $cand = Join-Path $sig $n
    if (Test-Path $cand) { $base = $cand; break }
}
if (-not $base) {
    Write-Host "[!] 리포트 원본 폴더를 못 찾음:`n    $sig\(THCI|TKS)데일리리포트`n    이 PC의 OneDrive 경로를 확인하세요." -ForegroundColor Yellow
    exit 1
}
